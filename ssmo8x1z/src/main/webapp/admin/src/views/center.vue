  ...
  <div>
    ...
    >  
     <el-row>
      ...
      <el-col :span="12">
        <el-form-item   v-if="flag=='dudao'"  label="督导工号" prop="dudaogonghao">
          <el-input v-model="ruleForm.dudaogonghao" readonly              placeholder="督导工号" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='dudao'"  label="督导姓名" prop="dudaoxingming">
          <el-input v-model="ruleForm.dudaoxingming"               placeholder="督导姓名" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item v-if="flag=='dudao'"  label="性别" prop="xingbie">
          <el-select v-model="ruleForm.xingbie" placeholder="请选择性别">
            <el-option
                v-for="(item,index) in dudaoxingbieOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
      ...
      <el-col :span="24">  
        <el-form-item v-if="flag=='dudao'" label="照片" prop="zhaopian">
          <file-upload
          tip="点击上传照片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.zhaopian?ruleForm.zhaopian:''"
          @change="dudaozhaopianUploadChange"
          ></file-upload>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='dudao'"  label="督导电话" prop="dudaodianhua">
          <el-input v-model="ruleForm.dudaodianhua"               placeholder="督导电话" clearable></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="12">
        <el-form-item   v-if="flag=='dudao'"  label="督导邮箱" prop="dudaoyouxiang">
          <el-input v-model="ruleForm.dudaoyouxiang"               placeholder="督导邮箱" clearable></el-input>
        </el-form-item>
      </el-col>
...
export default {
  ...
  mounted() {
    ...
    this.dudaoxingbieOptions = "男,女".split(',')
  },
  methods: {
    ...
    dudaozhaopianUploadChange(fileUrls) {
        this.ruleForm.zhaopian = fileUrls;
    },
    onUpdateHandler() {
      ...
      if((!this.ruleForm.dudaogonghao)&& 'dudao'==this.flag){
        this.$message.error('督导工号不能为空');
        return
      }
      if((!this.ruleForm.mima)&& 'dudao'==this.flag){
        this.$message.error('密码不能为空');
        return
      }
      if((!this.ruleForm.dudaoxingming)&& 'dudao'==this.flag){
        this.$message.error('督导姓名不能为空');
        return
      }
      if( 'dudao' ==this.flag && this.ruleForm.dudaodianhua&&(!isMobile(this.ruleForm.dudaodianhua))){
        this.$message.error(`督导电话应输入手机格式`);
        return
      }
      if( 'dudao' ==this.flag && this.ruleForm.dudaoyouxiang&&(!isEmail(this.ruleForm.dudaoyouxiang))){
        this.$message.error(`督导邮箱应输入邮箱格式`);
        return
      }
